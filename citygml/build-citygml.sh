...
if [ -z ${OCM_COUNTY+x} ]; then
    ...
    echo "Creating CityGML files for $OCM_STATE/$OCM_COUNTY and saving output to s3://$OCM_GML_S3BUCKET/"
fi
...
aws s3 cp s3://${OCM_BLDGS_S3BUCKET}/${OCM_STATE}/cty=${OCM_COUNTY} ${OCM_BLDGSFILES}/ --recursive
